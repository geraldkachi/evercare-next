        ...
        {/* section one */}
          ...
          <div className="flex items-center gap-2 mb-4">
            {dietaryArray.map(({value}) => {
              ...
              return (
                ...
                className={`${dietaryPreference.includes(value) && '!bg-[#F0E4FF] !border !border-[#9164CD]' }  cursor-pointer border border-[#1c1c1c] text-xs px-4 py-2 rounded-3xl max-w`}>{value}</div>
              )
            })}
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 mb-4">
            {physicalActivityData.map(({value}) => {
              ...
              return (
                ...
                className={`${physicalActivity.includes(value) && '!bg-[#F0E4FF] !border !border-[#9164CD]' } whitespace-nowr cursor-pointer border border-[#1c1c1c] text-xs px-4 py-2 rounded-3xl max-w`}>{value}</div>
              )
            })}
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 mb-4">
            {sleepPatternsData.map(({value}) => {
              ...
              return (
                <div key={value} onClick={RBoxgrouplogic}
                className={`${sleepPatterns.includes(value) && '!bg-[#F0E4FF] !border !border-[#9164CD]' } whitespace-nowrap cursor-pointer border border-[#1c1c1c] text-xs px-4 py-2 rounded-3xl max-w`}>{value}</div>
              )
            })}
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 mb-4">
            {smokeTobaccoProductsData.map(({value}) => {
              ...
              return (
                <div key={value} onClick={RBoxgrouplogic}
                className={`${tobaccoProducts.includes(value) && '!bg-[#F0E4FF] !border !border-[#9164CD]' } whitespace-nowrap cursor-pointer border border-[#1c1c1c] text-xs px-4 py-2 rounded-3xl max-w`}>{value}</div>
              )
            })}
          </div>
